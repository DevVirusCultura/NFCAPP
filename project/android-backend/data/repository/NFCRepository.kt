...
/**
 * Repository per gestione dati NFC
 */
@Singleton
class NFCRepository @Inject constructor(
    ...
    fun getTagByIdLiveData(tagId: String): LiveData<NFCTag?> = nfcTagDao.getTagByIdLiveData(tagId)
    
    fun searchTags(query: String): Flow<List<NFCTag>> = nfcTagDao.searchTags(query)
    
    fun getVirtualTags(): Flow<List<NFCTag>> = nfcTagDao.getVirtualTags()
    
    suspend fun getTagCount(): Int = nfcTagDao.getTagCount()
    
    suspend fun getTodayTagCount(): Int = nfcTagDao.getTodayTagCount()
    ...
    suspend fun insertTags(tags: List<NFCTag>) = nfcTagDao.insertTags(tags)
    ...
    suspend fun deleteAllVirtualTags() = nfcTagDao.deleteAllVirtualTags()
    
    suspend fun getRecentTags(limit: Int): List<NFCTag> = nfcTagDao.getRecentTags(limit)
    
    suspend fun limitHistorySize(maxSize: Int) = nfcTagDao.limitHistorySize(maxSize)
    ...
    fun getStatsLiveData(): LiveData<NFCStats?> = statsDao.getStatsLiveData()
    ...
    fun getSettingsLiveData(): LiveData<AppSettings?> = settingsDao.getSettingsLiveData()
    ...
    suspend fun exportData(): String {
        ...
        val exportData = mapOf(
            ...
            "version" to "1.0.0",
            "totalTags" to tags.toString(), // In real implementation, collect flow
